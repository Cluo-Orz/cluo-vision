...
- 已覆盖登录/注册、本地登录态恢复、首页、统一发现搜索、最近搜索、库内命中、番剧搜索与订阅、订阅/规则状态、下载列表、下载自动刷新、等待/错误状态展示、下载暂停/恢复、local-dev 下载完成、完成下载自动尝试入库、批量入库手动重试、入库后跳转详情、媒体库、详情、海报展示、播放会话、播放启动桥接、外部播放器安装/intent 检测、外部播放器返回后按离开时长回写进度、播放停止/标记看完、续播位置展示、播放状态写回媒体库、external-player intent 目标展示、历史、底层服务配置、Jellyfin 登录配置和系统诊断
...
9. ✅ Flutter analyze / test
10. ✅ Android SDK 打包和 debug APK 构建
11. ⬜ 安装到 Android TV、external-player 实机验证
